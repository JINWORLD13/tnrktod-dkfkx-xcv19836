  ...
  async getUserById(req, res, next) {
    try {
      ...
      if (req?.isAuthenticated() === true) {
        const cachedUser = await redisClient.get(`userInfo:${userId}`);
        if (cachedUser) {
        ...
        await redisClient.set(`userInfo:${userId}`, finalUserInfo, 3600); 
  ...
  async deleteUser(req, res, next) {
    try {
      ...
      if (req?.isAuthenticated() === true) {
        ...
        await redisClient.del(`tarotHistory:${userId}`);
        const userInDB = await userService.getUserById(userId);
        if (userInDB === undefined || userInDB === null) {
